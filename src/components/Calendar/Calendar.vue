<template>
  <div :class="[{showWeek:isWeek},'calendar-pro']" :style="{width,height}">
    <title-bar :year.sync="renderYear"
      :month.sync="renderMonth"
      v-if="showTitle"
    ...
    <div class="calendar-pro__head">
      <table width="100%" cellspacing="0" cellpadding="0">
        <thead>
          <tr align="center">
            <th v-for="(weekDay,i) in this.weekTitle" :key="i">周{{weekDay}}</th>
          </tr>
        </thead>
      </table>
    </div>
    ...
    renderContent: {
      ...
      default: function(h, data) {
        ...
        var $festival = festival ? (
          <div class="item">{festival}</div>
        ) : null;
        var $lunarFestival = lunarFestival ? (
          <div class="item">{lunarFestival}</div>
        ) : null;
        var $term = term ? <div class="info-term">{term}</div> : null;
        var $date = <span>{day}</span>;
        var $lunar = lunar === '初一' ? lunarMonth + lunar : lunar

        var _showLunar = this.showLunar
        if (this.mixinFestival) {
          _showLunar = _showLunar && !($festival || $lunarFestival)
        }

        return (
          <div class={boxClassName}>
            <div class="info-date">{$date}</div>
            {this.showFestival && ($festival || $lunarFestival) ?
